...
class DoerschDataset(data.Dataset):
    ...
    # base_dataset already constructed with config
        ...
        if config.include_rgb and self.base_dataset.purpose == "train":
            ...
            self.stddev_fname = os.path.join(config.doersch_stats, "%s_stats.pickle" % config.dataset)
            ...
            if not os.path.exists(self.stddev_fname):
                ...
                print("created Doersch dataset wrapping %s, stddev %s" % (config.dataset, self.stddev))
                sysout.flush()
    ...
    def __getitem__(self, index):
        ...
        if self.base_dataset.purpose == "test":
            ...
            noise = torch.zeros((2, self.input_sz, self.input_sz), dtype=torch.float32).cuda()
